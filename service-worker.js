const CACHE = 'ichigo-beauty-shell-v7';
const SHELL = [
  './',
  './index.html',
  './style.css',
...
const SHELL_FILES = new Set(['index.html','style.css','app.js','online.js','product-guide.js','product-specific.js','manifest.json']);
...
self.addEventListener('fetch', event => {
  ...
  if (req.mode === 'navigate' || /\/(app|online|product-guide|product-specific)\.js$/.test(url.pathname)) {
    event.respondWith(
      fetch(req)
        .then(res => {
          if (res.ok) {
